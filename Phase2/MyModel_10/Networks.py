...
def SpinalResNet18(device):
    return SpinalResNet(BasicBlock, [2,2,2,2], num_classes=18).to(device)

def ResNet34(device):
    return ResNet(BasicBlock, [3, 4, 6, 3]).to(device)

def SpinalResNet34(device):
    return SpinalResNet(BasicBlock, [3, 4, 6, 3], num_classes=18).to(device)