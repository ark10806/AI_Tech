...
obj_pth = os.path.join(base, 'ckpt/0822012621/SimCLR_0822012759.pth')
obj = torch.load(obj_pth)
# obj = torch.load(base + 'ckpt/0822012621/SimCLR_0822012759.pth')
...
def test_classifier(epoch, epochs):
    ...
    if acc > best_acc:
        ...
        model_path = os.path.join(ckpt_dir, f"{opt.}_{datetime.now().strftime('%m%d%H%M%S')}.pth")
